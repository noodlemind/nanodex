...
class ModelLoader:
    ...
    def apply_lora(self, model):
        ...
        try:
            ...
            logger.error(
                f"Failed to apply LoRA with target_modules={target_modules}: {e}",
                exc_info=True
            )
            raise RuntimeError(
        ...
        logger.info(f"LoRA applied to {len(lora_modules)} module(s): {lora_modules[:5]}{'...' if len(lora_modules) > 5 else ''}")
